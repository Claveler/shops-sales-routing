...
- **Timeslot Selector modal**: opened via the calendar button in the filter row; allows the cashier to pick a date and session for the active event before selling tickets. Follows the cinema/showtime-strip paradigm optimized for speed at the register:
  ...
  - **Horizontal date pill strip with calendar fallback**: only dates with at least one session are shown as pills (no blank calendar grid). If an event only runs on Wed + Fri, only those days appear. First available date is pre-selected. **At most 5 date pills** are shown to keep the row compact; if the event has more dates, a right-aligned **"More dates"** text link appears below the strip. If the active date falls outside the visible 5, it replaces the last pill so the selection is always visible. Tapping "More dates" switches to a **month-view mini calendar** inline (Monday-start week, three-letter weekday labels: MON TUE WED...). Available dates have a light-blue background; dates with constrained availability show a short colored underline bar beneath the date number (amber for "Filling up", red for "Low availability"). A legend below the grid explains the bar colors and sold-out indicator. The calendar supports month-to-month navigation via pill-shaped month buttons. Selecting a date from the calendar keeps the calendar open and auto-scrolls to the timeslots below so the cashier can pick a session without losing their place. When the calendar is open, the "Select a date" label becomes a back button ("← Select a date") that returns to the pill strip without changing the selection. Date pills that have a previously confirmed timeslot show a small check indicator absolutely positioned at the bottom of the pill, so it never changes the pill's dimensions.
